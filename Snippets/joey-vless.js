...
// --- 硬编码配置 ---
const authToken = '13383918-1256-4c1e-bfd8-62e82421dabe';
let fallbackAddress = '';
let fallbackPort = '443';
const socks5Config = '';
const manualWorkerRegion = 'US';  // 手动指定地区（留空则自动检测，可选值：US、SG、JP、HK、KR、DE、SE、NL、FI、GB）
const customPath = '6889692';// 短地址（自定义路径，留空则使用UUID路径，支持多级路径如：mypath 或 path/to/sub）
const githubPreferredURL = 'https://raw.githubusercontent.com/6889692/wiki/main/ip.txt'; // GitHub订阅URL（硬编码）
const enableGitHubPreferred = true;// 启用GitHub优选IP（true启用，false禁用）
const enableOtherPreferred = true;// 启用其他优选（域名优选，true启用，false禁用）
const apiBaseUrl = 'https://subapi.mgt.xx.kg/sub';// API地址配置（订阅转换服务）

const directDomains = [
    { name: "CF", domain: "ara.ns.cloudflare.com" }, 
    { domain: "desi.ns.cloudflare.com" }, { domain: "david.ns.cloudflare.com" }, 
    { domain: "eva.ns.cloudflare.com" }, { domain: "coco.ns.cloudflare.com" },
    { domain: "daphne.ns.cloudflare.com" }, { domain: "ace.ns.cloudflare.com" }, 
    { domain: "jay.ns.cloudflare.com" }, { domain: "aldo.ns.cloudflare.com" },
    { domain: "asa.ns.cloudflare.com" }, { domain: "dom.ns.cloudflare.com" }
];
...
export default {
	async fetch(request, env, ctx) {
		try {
			...
			// 根据地区智能选择备用IP作为访问失败时的备用（每次请求都重新选择）
